...
async function getStarsThisYear(owner: string, repo: string): Promise<number> {
	...
	while (true) {
		...
		if (!response.ok) {
			...
			throw new Error(
				`Failed to fetch stargazers for ${owner}/${repo}: ${response.statusText}`,
			);
		}
...
if (import.meta.main) {
	...
	console.log(
		`⭐ Checking ${starredRepos.length} repositories with stars...\n`,
	);
	...
	// Calculate totals
	const totalStarsThisYear = results.reduce(
		(sum, r) => sum + r.starsThisYear,
		0,
	);
	...
	if (reposWithNewStars.length > 0) {
		...
		const maxStarsLen = Math.max(
			...top10.map((r) => String(r.starsThisYear).length),
		);
		const maxTotalLen = Math.max(
			...top10.map((r) => String(r.totalStars).length),
		);
